...
async function initSchema() {
  ...
  const indexes = [
    ...
    // Index DPE
    'CREATE INDEX IF NOT EXISTS idx_biens_dpe ON biens(dpe_classe) WHERE dpe_classe IS NOT NULL',
    'CREATE INDEX IF NOT EXISTS idx_acq_criteria_dpe ON acquereur_criteria USING GIN (dpe_classes)',
  ];
...
async function matchAcquereurToBiens(acquereurId, hideelegation = true) {
  ...
  if (criteria) {
    ...
    if (criteria.dpe_classes) {
      try {
        const classes = JSON.parse(criteria.dpe_classes);
        if (Array.isArray(classes) && classes.length > 0) {
          // Filtre éliminatoire : exclut les biens dont le DPE est renseigné mais incompatible.
          // Les biens sans dpe_classe (NULL) passent toujours (données incomplètes tolérées).
          criteriaConditions.push(`(b.dpe_classe IS NULL OR b.dpe_classe IN (${classes.map(() => `$${paramIdx++}`).join(',')}))`);
          criteriaParams.push(...classes);
        }
      } catch (_) {}
    }
...
async function matchBienToAcquereurs(bienId, ownerEmail = null) {
  ...
  return acquereurs.filter(a => {
    try {
      ...
      if (a.dpe_classes) {
        try {
          const classes = JSON.parse(a.dpe_classes);
          // Filtre éliminatoire uniquement si le bien a un DPE renseigné
          if (Array.isArray(classes) && classes.length > 0 && bien.dpe_classe) {
            if (!classes.includes(bien.dpe_classe)) return false;
          }
        } catch (_) {}
      }
      if (a.secteurs) {

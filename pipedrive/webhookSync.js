...
function normalizeEventType(eventType) {
  ...
  if (parts.length !== 2) return { action: eventType, object: null };
  const [first, second] = parts;
  if (['deal', 'person', 'activity'].includes(first)) return { action: second, object: first };
  return { action: first, object: second };
}

async function syncWebhookToDb(eventType, payload) {
  const { action, object } = normalizeEventType(eventType);
  const deal = payload?.current || payload?.previous || payload?.data || payload;
  if (!deal?.id) {
    logger.warn('Webhook ignore: payload sans id', { eventType });
    return { skipped: true, reason: 'missing_id' };
  }

  if (object && object !== 'deal') {
    logger.info('Webhook ignore: type non gere par la synchro deal', { eventType, object });
    return { skipped: true, object };
  }

  if (action === 'deleted' || deal.status === 'deleted' || deal.status === 'lost') {
    await archiveDeal(deal.id);
    return { archived: true, dealId: deal.id };
  }

  const { bienStageId, acqStageId } = getCachedStageIds();
  if (bienStageId && Number(deal.stage_id) === Number(bienStageId)) {
    await syncSingleBien(deal, config.PIPEDRIVE_API_TOKEN);
    return { synced: 'bien', dealId: deal.id };
  }
  if (acqStageId && Number(deal.stage_id) === Number(acqStageId)) {
    await syncSingleAcquereur(deal);
    return { synced: 'acquereur', dealId: deal.id };
  }

  const { rows: existingBiens } = await pool.query(
    'SELECT id FROM biens WHERE pipedrive_deal_id = $1 AND archived = 0',
    [deal.id]
  );
  const { rows: existingAcquereurs } = await pool.query(
    'SELECT id FROM acquereurs WHERE pipedrive_deal_id = $1 AND archived = 0',
    [deal.id]
  );

  if (existingBiens.length) {
    await syncSingleBien(deal, config.PIPEDRIVE_API_TOKEN);
    return { synced: 'bien', dealId: deal.id };
  }
  if (existingAcquereurs.length) {
    await syncSingleAcquereur(deal);
    return { synced: 'acquereur', dealId: deal.id };
  }

  logger.info('Webhook ignore: deal hors stages cibles et inconnu localement', {
    eventType,
    dealId: deal.id,
    stageId: deal.stage_id,
  });
  return { skipped: true, reason: 'unknown_stage', dealId: deal.id };
}

module.exports = { syncSingleBien, syncSingleAcquereur, syncWebhookToDb };
